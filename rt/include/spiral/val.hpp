...
namespace spiral {
  ...
  struct Val {
    ...
    static inline auto wrap_fun(uint32_t* ptr) -> Val;
    static inline auto wrap_fun(void* ptr) -> Val;
    static inline auto wrap_data_obj(uint32_t* ptr) -> Val;
    static inline auto wrap_data_obj(void* ptr) -> Val;
  ...
  auto Val::wrap_fun(void* ptr) -> Val {
    return Val::wrap_fun(static_cast<uint32_t*>(ptr));
  }
  auto Val::wrap_data_obj(uint32_t* ptr) -> Val {
    assert(ptr != 0);
    assert((reinterpret_cast<uint32_t>(ptr) & 0b11) == 0);
    return Val(reinterpret_cast<uint32_t>(ptr) + 0b11);
  }
  auto Val::wrap_data_obj(void* ptr) -> Val {
    return Val::wrap_data_obj(static_cast<uint32_t*>(ptr));
  }
